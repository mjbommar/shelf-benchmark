...
def main():
    record_sets_dir = Path(
        "/nas3/data/legal/us/cataloging-records-all-cgp-utf8/Record_sets"
    )
    ...
    observed = sum(
        ...
        for lcc in top_lcc
        for g in top_genres
        if results["lcc_genre"].get((lcc, g), 0) > 0
    )
    ...
    observed = sum(
        ...
        for lcc in top_lcc
        if results["agency_lcc"].get((a, lcc), 0) > 0
    )
    print(f"  Possible cells: {possible}")
    print(f"  Observed cells: {observed} ({observed / possible * 100:.1f}%)")
